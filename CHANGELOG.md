...
- **CUDA builds required AVX-512 on the machine that ran them.** The same `GGML_NATIVE` default adds `-march=native`, baking the build runner's CPU ISA into the release: the v0.0.25 CUDA binary contains 6191 AVX-512 register references while the CPU and Vulkan builds of the same release contain none. On a CPU without AVX-512 that is a SIGILL at startup. The ISA baseline is now deterministic, and CI fails the build if AVX-512 appears.
- **The CUDA and Vulkan `.deb` packages declared almost no dependencies.** `dpkg-shlibdeps` cannot resolve the GPU libraries at package time and `$auto` gives up entirely rather than degrading, so those packages shipped with `Depends: ffmpeg` — losing libc, libstdc++, ALSA, and OpenSSL. Spelled out explicitly.
