...
- **CUDA builds shipped kernels for a single GPU architecture.** ggml defaults `GGML_NATIVE=ON`, which sets `CMAKE_CUDA_ARCHITECTURES` to `native`; on a GPU-less CI runner that collapses to nvcc's single default architecture. Every published `ostt-cuda` binary through v0.0.25 contains `sm_52` SASS only, so GTX 750-class cards got no kernel image at all and everything newer ran JIT-compiled PTX with no architecture-tuned kernels. The builds now pin the architecture list, and CI asserts the embedded architectures.
- **CUDA builds required AVX-512 on the machine that ran them.** The same `GGML_NATIVE` default adds `-march=native`, baking the build runner's CPU ISA into the release: the v0.0.25 CUDA binary contains 6191 AVX-512 register references while the CPU and Vulkan builds of the same release contain none. On a CPU without AVX-512 that is a SIGILL the first time the ggml code runs — at model load or inference, not at startup, so `ostt --version` passes on such a machine. The ISA baseline is now deterministic, and CI fails the build if AVX-512 appears. One consequence for the CUDA 12 build: it now carries native kernels for Ampere and newer plus PTX for older architectures, so Maxwell and Pascal cards JIT-compile from PTX on first use where previously only Maxwell ran natively.
- **The CUDA `.deb` packages declared almost no dependencies.** `dpkg-shlibdeps` cannot find `libcuda.so.1` at package time (only the build stub exists there) and `$auto` gives up entirely rather than degrading, so `ostt-cuda` shipped with `Depends: ffmpeg` — losing libc, libstdc++, ALSA, and OpenSSL. The Vulkan package was unaffected. The CUDA packages now declare the base libraries explicitly, with the pre- and post-`t64` package names as alternatives so they install on Ubuntu 24.04 and Debian 13 as well as older releases, and CI asserts that every package's `Depends` names `libc6`.
- **`ostt --version` could not tell the CUDA 12 and CUDA 13 builds apart** — both reported `-cuda`, because the suffix was keyed on the shared Cargo feature. The release workflow now names the variant explicitly and the binary reports `-cuda13`.
- **The package variants overlapped without declaring it.** `ostt`, `ostt-cuda`, `ostt-cuda13`, and `ostt-vulkan` all own `/usr/bin/ostt` but declared no `Conflicts`/`Replaces`/`Provides`, so switching between them failed in dpkg and left two binaries on the system. Now declared for both `.deb` and `.rpm`.
